...
Simulador de compatibilidade entre o perfil de um candidato e vagas de Front-End, construído com **HTML, CSS e JavaScript puro** (ES Modules), sem frameworks.
...
| Sem geolocalização | Serviço opcional de **geolocalização** para dar contexto sobre remoto x presencial |
...
> ⚠️ O projeto usa `fetch` para carregar `assets/dados/vagas.json`, então precisa ser servido por um servidor local — abrir o `index.html` direto via `file://` bloqueia o `fetch` por política de CORS do navegador.
...
- **Geolocalização (opcional)**: `services/geolocation.js` pergunta a localização ao navegador (com permissão do usuário) e exibe uma nota contextual sobre vagas remotas; falha de forma silenciosa se o usuário negar ou o navegador não suportar, nunca bloqueando o restante da aplicação.
